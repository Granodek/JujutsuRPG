      ...
      <div className="bg-[#f9c87f] rounded-xl p-5 text-center shadow-lg flex-1">
        ...
        {Object.entries(character.stats).map(([stat, value]) => {
          ...
          if (stat === 'hp') {
            ...
            barColor = stat === 'energy' ? 'bg-[#5ae9dd]' : stat === 'stamina' ? 'bg-orange-600' : stat==='shield' ? 'bg-yellow-600' : 'bg-purple-600';
          }
        ...
        <div className="flex flex-col mt-4">
          {character.name !== 'Malzahar' ? (
                ...
                {stat === 'hp'?(<div>
                  <input 
                    ...
                    onChange={(e) => setNumero(Number(e.target.value))} 
                     />    
                  <button onClick={() => handleInputChange(stat, character.stats[stat] -  Math.max(0,(numero -((character.maxStats['stamina']*10)/2) )))} type='submit'>Calcular</button>
                  </div>) : <div></div> 
                }
                {stat === 'energy'?(<div>
                  <input 
            ...
            <>
              <div className="flex flex-col mt-4">
                {['hp', 'shield', 'DarkHp', 'stamina', 'energy', 'panic'].map((stat) => (
                  <div className="flex flex-row mt-2 w-full" key={stat}>
                    <label className="font-bold w-[20%]" htmlFor={`${stat}-change`}>
                      {stat.charAt(0).toUpperCase() + stat.slice(1)}:
                    </label>
                ...
                {stat === 'DarkHp'?(<div>
                  <input 
                    ...
                    onChange={(e) => setdarkNumero(Number(e.target.value))} 
                     />    
                  <button onClick={() => handleInputChange(stat, character.stats[stat] -  Math.max(0,(darknumero -((character.maxStats['stamina']*10)/2) )))} type='submit'>Calcular</button>
                  </div>) : <div></div> 
                }
                {stat === 'energy'?(<div>
                  <input 
