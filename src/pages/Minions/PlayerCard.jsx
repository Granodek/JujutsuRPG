...
const Character = ({ characters, selectedCharacter, onSelectCharacter, onUpdateStat, onUpdateMaxStat }) => {
  ...
  const handleInputChange2 = (stat, value, value2) => {
    onUpdateStat(stat, value2);
  };

  const handleMaxInputChange = (stat, value, value2) => {
    onUpdateMaxStat(stat, value2);
    handleInputChange2(stat, value2, value)
  };
  ...
  return (
    <div className="bg-[#f9c87f] rounded-xl p-5 shadow-lg max-w-6xl w-[95%] flex gap-5 mx-auto">
      ...
      <div className="bg-[#f9c87f] rounded-xl p-5 text-center shadow-lg flex-1">
        <div className="mb-5">
          <label htmlFor="character-select" className="text-xl font-bold">Escolha o Minion</label>
          <select
            id="character-select"
        ...
        {Object.entries(character.stats).map(([stat, value]) => {
          ...
          if (stat === 'hp') {
            ...
            barColor = stat === 'energy' ? 'bg-[#5ae9dd]' : stat === 'stamina' ? 'bg-orange-600' : stat==='shield' ? 'bg-yellow-600' : stat==='Chaos' ? 'bg-yellow-400' : 'bg-purple-600';
          }
              ...
              <div className="flex items-center mb-2.5">
                ...
                {stat === 'hp' ? (
                    <>
                      ...
                      ></div>
                      <div
                        className="absolute bottom-0 w-full h-2/4 bg-yellow-600 border-2 border-black border-solid rounded-xl transition-all duration-700"
                        style={{ width: `${(character.stats.shield / character.maxStats.shield) * 100}%` }}
                      ></div>
                    </>
                  ) : stat !== 'DarkHp' ? (
                  ...
                  <div className="absolute inset-0 flex items-center justify-center text-white font-bold">
                    {stat !== 'DarkHp' && stat !== 'shield' ? `${value}/${character.maxStats[stat]}`: null}
            ...
            <>
              <div className="flex flex-col mt-4">
                ...
                {stat === 'stamina'?(<div>
                  ...
                  <button onClick={() => handleMaxInputChange(stat, 30, 30)} type='submit'>Reset</button>
                  <button className='px-2'>ou</button>
                  <button onClick={() => handleMaxInputChange(stat, character.stats[stat] +  6, character.maxStats[stat]+6)} type='submit'>Aumentar</button>
                  </div>) : <div></div> 
                }
                {stat === 'energy'?(<div>
                  <input 
...
//descobri sem querer que 500 de energia defende o mesmo que 100 de resistencia