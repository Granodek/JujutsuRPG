...
const Equipment = ({ equipment, level, grade, effects }) => {
  ...
  return (
    <div className="p-6 border border-gray-700 bg-gray-900 rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold text-center text-blue-400 mb-4">Equipamentos</h2>

     
      <div className="text-center mb-4">
        <p className="text-xl font-semibold">Nível: <span className="text-green-400">{level}</span></p>
        <p className="text-xl font-semibold">Grau: <span className="text-purple-400">{grade}</span></p>
      </div>

      <div className="mb-6">
        <h3 className="text-lg font-semibold border-b border-gray-600 pb-2 mb-2">Efeitos Ativos</h3>
        <ul className="list-disc list-inside text-gray-300">
          {effects.length > 0 ? (
            effects.map((effect, index) => (
              <li key={index} className="text-yellow-400">{effect}</li>
            ))
          ) : (
            <li className="text-gray-500">Nenhum efeito ativo</li>
          )}
        </ul>
      </div>
